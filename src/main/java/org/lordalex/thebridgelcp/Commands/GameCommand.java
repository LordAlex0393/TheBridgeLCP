...
//                    }
                    ...
                    if (sender instanceof Player) {
                        Player p = (Player) sender;
                        p.sendMessage(ColorUtil.get("&cИгра ещё не запущена"));
                    }
                }
                    ...
                    if (args[1].equalsIgnoreCase("max-points")) {
                        if (args.length > 2) {
                            try {
                                ...
                                if (sender instanceof Player) {
                                    Player p = (Player) sender;
                                    p.sendMessage(ColorUtil.get("&Количество очков должно быть числом"));
                                }
                            }
                        }
                        ...
                        if (args.length > 2) {
                            try {
                                ...
                                if (sender instanceof Player) {
                                    Player p = (Player) sender;
                                    p.sendMessage(ColorUtil.get("&Количество игроков должно быть числом"));
                                }
                            }
                        }
                    }
    ...
    private static void printCommandInfo(Player p) {
        p.sendMessage(ColorUtil.get("&e---------- &dУправление игрой&f (&e/game&f)&e ----------"));
        p.sendMessage(ColorUtil.get("&e/game&7 start&f: запустить игру"));
        p.sendMessage(ColorUtil.get("&e/game&7 stop&f: остановить игру"));
        p.sendMessage(ColorUtil.get("&e/game&7 flag&f: управление настройками сервера"));
    ...
    private static void printFlagInfo(Player p) {
        p.sendMessage(ColorUtil.get("&e---------- &dНастройки сервера&f (&e/game flag&f)&e ----------"));
        p.sendMessage(ColorUtil.get("&dmax-points&f (&7Количество очков необходимых для победы&f): " + GameUtil.SCORES_TO_WIN));
        p.sendMessage(ColorUtil.get("&dmax-players&f (&7Количество игроков необходимых для старта игры&f): " + TheBridgeLCP.config.getPlayersToStart()));
    }
